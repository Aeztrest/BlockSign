// - Prefers JSON output but falls back to clean Markdown parsing when needed.
// - Contains resilient parsing helpers to unescape/clean the model output and normalize to GeneratedContract.
//
// Usage: call generateContract(params) from a Next.js API route (server-side). Set server env: GENAI_API_KEY (or GENERATIVE_API_KEY).
// Note: keep your API key server-side; do not expose NEXT_PUBLIC keys in production.
